...
% This is the code edited by Joel Frohlich to use IAAFT2 as the first
% surrogate algorithm (NOT AAFT), and to include an rng seed as input
...
% seed - rng seed to feed to surrogate
...
% Generate 1,000 Interative Amplitude Adjusted Fourier Transform surrogates and
% calculate their permutation entropies
try
    [surr, params] = surrogate_JF(surr_y, 1000, 'IAAFT2', 1, fs,seed);
catch
    [surr, params] = surrogate_JF(zscore(surr_y), 1000, 'IAAFT2', 1, fs,seed);
end
sig=params.cutsig;
for i = 1:1000
...
if stochastic1==1
    ...
    while surr_flag==0
        try
            [surr, params] = surrogate_JF(surr_y, 1000, 'CPP', 1, fs,seed);
        catch
            [surr, params] = surrogate_JF(zscore(surr_y), 1000, 'CPP', 1, fs,seed);
        end
        sig=params.cutsig;
        for i = 1:1000
            x1 = surr(i,:);
