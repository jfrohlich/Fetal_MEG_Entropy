function [ res ] = LZDecomposition_fMEG(x1, s1, x2, s2, nb_samples, fsample, group)
%% LZDECOMPOSITION Decomposes spectral, phasic, and interaction components
% of the LZ difference between two datasets.
%
%   R = LZDECOMPOSITION(X1, X2), where X1,X2 are 1D cell arrays of 1-by-T time
%   series vectors, computes the average difference in LZ(X1) - LZ(X2) and
%   decomposes it in spectral, phasic, and interaction terms using phase
%   randomisation techniques.
%
%   R = LZDECOMPOSITION(X1, X2, N) uses N surrogate samples (default: 1000).
%
%   R = LZDECOMPOSITION(..., S) appends the results to pre-existing struct S.
%
% Results are returned in a struct R with all elements of the decomposition.
% These are:
%
%   - lz1: average LZ of dataset X1
%   - lz2: average LZ of dataset X2
%   - diff: total LZ difference (X1 - X2)
%   - pai1: phase-amplitud component of LZ in X1
%   - pai2: phase-amplitud component of LZ in X2
%   - phase: phasic component of LZ difference (X1 - X2)
%   - spec: spectral component of LZ difference (X1 - X2)
%
% Reference:
...
LZ = @(v) LZ76(v >= median(v)); % - JF edit, 07/05/21
...
%% Pre-compute spectra, phases and true LZ
...
lz1 = mean(cellfun(LZ, x1));
lz2 = mean(cellfun(LZ, x2));


%% Initialise arrays and compute surrogate LZ values
%{
Notation:
...
%}
lz1_f1  = zeros([nb_samples, 1]);
lz1_f12 = zeros([nb_samples, 1]);
lz2_f2  = zeros([nb_samples, 1]);
lz2_f12 = zeros([nb_samples, 1]);
...
lz1_f1  = mean(lz1_f1);
lz1_f12 = mean(lz1_f12);
lz2_f2  = mean(lz2_f2);
lz2_f12 = mean(lz2_f12);
...
res.pai1 = lz1 - lz1_f1;
res.pai2 = lz2 - lz2_f2;
phase1 = lz1_f1 - lz1_f12;
phase2 = lz2_f2 - lz2_f12;
res.phase  = phase1 - phase2;
res.spec = lz1_f12 - lz2_f12; % the phases are pooled, so difference must be due to spectra
...
% res.pai1 = lz1 - lz1_f1;
% res.pai2 = lz2 - lz2_f2;
% res.phase1 = lz1_f1 - lz1_f12;
% res.phase2 = lz2_f2 - lz2_f12;
% res.phase  = res.phase1 - res.phase2;
% res.spec = lz1_f12 - lz2_f12;
