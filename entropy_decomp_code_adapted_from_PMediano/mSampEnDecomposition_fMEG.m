function [ res ] = mSampEnDecomposition_fMEG(x1, s1, x2, s2, nb_samples, fsample, group)
%% mSampEnDECOMPOSITION Decomposes spectral, phasic, and interaction components
% of the mSampEn difference between two datasets.
%
%   R = mSampEnDECOMPOSITION(X1, X2), where X1,X2 are 1D cell arrays of 1-by-T time
%   series vectors, computes the average difference in mSampEn(X1) - mSampEn(X2) and
%   decomposes it in spectral, phasic, and interaction terms using phase
%   randomisation techniques.
%
%   R = mSampEnDECOMPOSITION(X1, X2, N) uses N surrogate samples (default: 1000).
%
%   R = mSampEnDECOMPOSITION(..., S) appends the results to pre-existing struct S.
%
% Results are returned in a struct R with all elements of the decomposition.
% These are:
%
%   - pe1: average mSampEn of dataset X1
%   - pe2: average mSampEn of dataset X2
%   - diff: total mSampEn difference (X1 - X2)
%   - pai1: phase-amplitud component of mSampEn in X1
%   - pai2: phase-amplitud component of mSampEn in X2
%   - phase: phasic component of mSampEn difference (X1 - X2)
%   - spec: spectral component of mSampEn difference (X1 - X2)
%
% Reference:
...
se1_f1  = nan([nb_samples, 1]);
se1_f12 = nan([nb_samples, 1]);
se2_f2  = nan([nb_samples, 1]);
se2_f12 = nan([nb_samples, 1]);
...
se1 = mean(cellfun(SE, x1, CFG));
se2 = mean(cellfun(SE, x2, CFG));
...
se1_f1  = squeeze(mean(se1_f1));
se1_f12 = squeeze(mean(se1_f12));
se2_f2  = squeeze(mean(se2_f2));
se2_f12 = squeeze(mean(se2_f12));
...
res.pai1 = se1' - se1_f1;
res.pai2 = se2' - se2_f2;
phase1 = se1_f1 - se1_f12;
phase2 = se2_f2 - se2_f12;
res.phase  = phase1 - phase2;
res.spec = se1_f12 - se2_f12; % the phases are pooled, so difference must be due to spectra
